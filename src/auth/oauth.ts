...
const SINGLE_USER: OAuthUser = {
  ...
  hashedPassword: "", // Not used for API key auth
  scopes: ["read", "write"],
  profile: {},
  createdAt: new Date(),
  updatedAt: new Date(),
  ...
  oauth: new MCPOAuthServer(
    {
      ...
      auth: {
        authenticateUser: async (credentials: { username?: string; password?: string }) => {
          // For single-user API key authentication, we only check the password field
          const providedApiKey = (credentials?.password || "").trim();
...
<html>
  <head>
    ...
    <title>{{PROJECT_NAME}} – API Key Authentication</title>
    <style>
      ...
      * { box-sizing:border-box; }
      ...
      p { color:#4b5563; font-size:14px; line-height:1.5; margin-bottom:20px; text-align:center; }
      ...
      .info { background:#dbeafe; color:#1e40af; padding:12px; border-radius:var(--radius); margin-bottom:16px; text-align:center; font-size:13px; }
    </style>
  </head>
  <body>
    <form class="card" method="POST" action="/authorize">
      ...
      <div class="info">
        This is a single-user MCP server. Enter your API key to continue.
      </div>
      ${errorHtml}
      ...
      <div class="form-group">
        ...
        <input type="password" id="password" name="password" required placeholder="Enter your API key" />
      </div>
  ...
  userLookup: async (jwtPayload: any) => {
    // For single-user setup, always return the single user
    return {
      id: SINGLE_USER.id,
      username: SINGLE_USER.username,
      email: SINGLE_USER.email || "single-user@example.com", // Ensure email is always a string
      scopes: SINGLE_USER.scopes,
      profile: SINGLE_USER.profile,
    };
  },
};
